    ...
    def traverse_skeletons(skel_image, end_points, brh_points, foreground):
        ...
        if not end_points and not brh_points:
            ...
            if len(end_points) == 1:
                ...
                print("A branch with more than 2 points was ignored!")
...
ree > 2): Colored yellow (255, 255, 0)
    ...
    def smooth_skeleton_image(self, image, size = 1, sigma = 1):
        """
        smooth the skeleton image to compelete the edges
        
        """
        from skimage import morphology, filters
        from skimage.morphology import dilation
        
        dilated_skeleton = dilation(image, morphology.square(size))
        closed_skeleton = morphology.closing(dilated_skeleton, morphology.square(size))
        final_skeleton = filters.gaussian(closed_skeleton.astype(float), sigma=sigma) > 0.5
        print('skeleton was smoothed')
    ...
    def analyze_image(self, image):
        ...
        # skeleton = self.smooth_skeleton_image(skeleton)
        self.skel_image = (skeleton * self.FOREGROUND).astype(np.uint8)
        # return skeleton, self.smooth_skeleton_image(skeleton), self.FOREGROUND, self.BACKGROUND, self.branch_thresh, self.skel_thresh
