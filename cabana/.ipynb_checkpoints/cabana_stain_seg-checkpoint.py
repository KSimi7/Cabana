...
class Cabana_stain_analyzer:
    def __init__(self, param_file, input_images, stain_matrix, th = 0.5, ims_res = 1):
        ...
        self.color_segmentor = decon_segmentor(self.stain_matrix, self.th)
        self.detector = self.getFiberDetector()
        self.orient_analyzer = OrientationAnalyzer(2.0)
        self.skeletonAnalyzer = self.getSkeletonAnalyzer()
    ...
    def getSkeletonAnalyzer(self):
        """Get Skeleton Analyzer"""
    ...
    def get_psr_roi(self, image):
        ...
        roi = self.color_segmentor.generate_rois(image, mask.astype(np.uint8), thickness=1)
        return mask, roi 
    ...
    def get_Orientation_results(self, roi, mask):
        ...
        orientation_metrics['Orient. Alignment'] = self.orient_analyzer.mean_coherency(mask = mask)
